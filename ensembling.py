...
parser.add_argument('--confidence_threshold', type=float, default=0.968, help='confidence neede by teacher to influence student')
...
parser.add_argument('--ema_constant', type=float, default=0.5, help='exponential moving average constant of student to teacher model')
...
def _calculate_loss(
    student_start_logits, student_end_logits, teacher_start_logits, teacher_end_logits, start_positions, end_positions, confidence_threshold, eval=True
):
    """
    ...
    start_loss = criterion(student_start_logits, start_positions)
    end_loss = criterion(student_end_logits, end_positions)

    supervised_loss = (start_loss + end_loss) / 2.

    if eval:
        return supervised_loss

    softmax = nn.Softmax(dim=1)
    start_conf_max, _ = torch.max(softmax(teacher_start_logits), 1)
    end_conf_max, _ = torch.max(softmax(teacher_end_logits), 1)

    bounded_student_start_logits = torch.clone(student_start_logits)
    bounded_student_end_logits = torch.clone(student_end_logits)
    bounded_teacher_start_logits = torch.clone(teacher_start_logits)
    bounded_teacher_end_logits = torch.clone(teacher_end_logits)

    bounded_student_start_logits[bounded_student_start_logits == float('-inf')] = 0
    bounded_student_start_logits[bounded_student_start_logits == float('inf')] = 0
    bounded_student_end_logits[bounded_student_end_logits == float('-inf')] = 0
    bounded_student_end_logits[bounded_student_end_logits == float('inf')] = 0
    bounded_teacher_start_logits[bounded_teacher_start_logits == float('-inf')] = 0
    bounded_teacher_start_logits[bounded_teacher_start_logits == float('inf')] = 0
    bounded_teacher_end_logits[bounded_teacher_end_logits == float('-inf')] = 0
    bounded_teacher_end_logits[bounded_teacher_end_logits == float('inf')] = 0
    ...
    s_t_squared_diff = torch.pow(torch.cat((bounded_student_start_logits, bounded_student_end_logits), 1) - torch.cat((bounded_teacher_start_logits, bounded_teacher_end_logits), 1), 2)
    s_t_squared_diff[(start_conf_max < confidence_threshold) | (end_conf_max < confidence_threshold)] = 0
    s_t_squared_diff[start_positions != ignored_index] = 0

    unsupervised_loss = torch.sum(s_t_squared_diff)

    return supervised_loss + unsupervised_loss


def train(args, epoch, student_model, teacher_model, dataset):
    """
    ...
    for batch in train_dataloader:
        ...
        student_start_logits, student_end_logits = student_model(batch)
        teacher_start_logits, teacher_end_logits = teacher_model(batch)
        ...
        for key in student_state_dict:
            teacher_state_dict[key] = args.ema_constant * student_state_dict[key] + (1 - args.ema_constant) * teacher_state_dict[key]
...
def main(args):
    ...
    path_base = "ensembling_w_hdr_" + str(args.hidden_data_ratio)
    ...
    # Select model.
    student_model = _select_model(args)
    num_pretrained = student_model.load_pretrained_embeddings(
    ...
    student_model.load_state_dict(torch.load(args.pretrained_model_path))
    torch.save(student_model.state_dict(), model_path)

    if args.use_gpu:
        student_model = cuda(args, student_model)

    params = sum(p.numel() for p in student_model.parameters() if p.requires_grad)
    ...
    if args.do_train:
        ...
        eval_history = []
        best_eval_loss = evaluate(args, 0, student_model, dev_dataset)

        # Begin training.
        for epoch in range(1, args.epochs + 1):
            # Perform training and evaluation steps.
            train_loss = train(args, epoch, student_model, teacher_model, train_dataset)
            eval_loss = evaluate(args, epoch, student_model, dev_dataset)
    ...
    if args.do_test:
        ...
        write_predictions(args, student_model, model_path, predictions_path, dev_dataset)
        eval_cmd = (
            'python3 evaluate.py '
